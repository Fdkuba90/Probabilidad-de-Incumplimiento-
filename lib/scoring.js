...
export function puntuarSinAtrasos(val) {
  ...
  // 0 => 62; 1–3.99 => 50; 4–7.99 => 41; >=8 => 16
  {
    const n = toNum(val[1]) ?? 0;
    if (n === 0) pts[1] = 62;
    else if (n < 4) pts[1] = 50;
    else if (n < 8) pts[1] = 41;
    else pts[1] = 16;
  }
  ...
  {
    const m = toNum(val[16]) ?? 0;
    if (m < 24) pts[16] = 41;
    else if (m < 36) pts[16] = 51;
    else if (m < 98) pts[16] = 60;
    else if (m < 120) pts[16] = 61;
    else pts[16] = 67;
  }
  ...
  // 1–6.99 => 46; 0 o >=7 => 58
  {
    const m = toNum(val[17]) ?? 0;
    pts[17] = m > 0 && m < 7 ? 46 : 58;
  }
...
export function puntuarConAtrasos(val) {
  ...
  // 14) BK12_IND_QCRA — indicador quitas/castigos/restructuras
  // Sin info 49; 0 => 51; >=1 => 13
  {
    const n = toNum(val[14]);
    if (n === null) pts[14] = 49;
    else pts[14] = n === 0 ? 51 : 13;
  }

  // 5) % pagos en tiempo a instituciones NO bancarias
  ...
  // 4) Nº pagos en tiempo (últimos 12 meses)
