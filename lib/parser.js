...
export async function parsePdf(buffer) {
  ...
  // A) Coordenadas (acepta formato viejo y nuevo)
  try {
    const byCoords = await extractCreditosActivosByCoords(buffer);

    if (byCoords && (byCoords.original != null || byCoords.vigente != null)) {
      // Soporta ambos formatos: buckets.v*  o  d1_29/d30_59/...
      let buckets = byCoords.buckets;
      if (!buckets && ('d1_29' in byCoords || 'd30_59' in byCoords)) {
        buckets = {
          v1_29:     byCoords.d1_29,
          v30_59:    byCoords.d30_59,
          v60_89:    byCoords.d60_89,
          v90_119:   byCoords.d90_119,
          v120_179:  byCoords.d120_179,
          v180p:     byCoords.d180_plus,
        };
      }

      const d1_29     = Math.round((buckets?.v1_29    ?? 0) || 0);
      const d30_59    = Math.round((buckets?.v30_59   ?? 0) || 0);
      const d60_89    = Math.round((buckets?.v60_89   ?? 0) || 0);
      const d90_119   = Math.round((buckets?.v90_119  ?? 0) || 0);
      const d120_179  = Math.round((buckets?.v120_179 ?? 0) || 0);
      const d180_plus = Math.round((buckets?.v180p    ?? 0) || 0);
