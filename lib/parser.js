...
import { extractActivosResumen } from "./activosResumen";
...
/* ========= Califica/Calificador (fallback por texto) ========= */
const KNOWN_CODES = new Set([
  "BK12_CLEAN","BK12_NUM_CRED","BK12_NUM_TC_ACT","NBK12_NUM_CRED",
  "BK12_NUM_EXP_PAIDONTIME","BK12_PCT_PROMT","NBK12_PCT_PROMT",
  "BK12_PCT_SAT","NBK12_PCT_SAT",
  "BK24_PCT_60PLUS","NBK24_PCT_60PLUS","BK12_PCT_60PLUS",
  "BK12_PCT_90PLUS","NBK12_PCT_90PLUS",
  "NBK12_COMM_PCT_PLUS","BK12_DPD_PROM","BK12_IND_QCRA",
  "BK12_MAX_CREDIT_AMT","MONTHS_ON_FILE_BANKING","MONTHS_SINCE_LAST_OPEN_BANKING"
]);
...
function extractCalificaRows(text) {
  const rows = [];
  let scope = text;
  // Busca "Califica" (histórico) cercano al bloque
  const idx = text.search(/Califica(?:dor)?\s*\n/i);
  if (idx >= 0) scope = text.slice(idx, idx + 8000);

  const lineRe = /^\s*([0-9]{1,3})\s+([A-Z0-9_]{2,})\s+([^\n\r]+?)\s*$/gmi;
  let m;
  while ((m = lineRe.exec(scope))) {
    const id = Number(m[1]);
...
/* ========= Totales por texto (último recurso) ========= */
function extractTotalesRow(text) {
  ...
  // buscar ventanas que contengan al menos 8 números (O + V + 6 buckets)
...
export async function parsePdf(buffer) {
  ...
  // A) NUEVO: intentar primero el Resumen (incluye "modo Bridova")
  let tot = await extractActivosResumen(buffer).catch(() => null);

  // B) Si no, usar coordenadas de la tabla clásica
  if (!tot) tot = await extractCreditosActivosByCoords(buffer).catch(() => null);

  // C) Fallback por texto (sumas por filas)
  if (!tot) tot = sumCreditosActivosByRows(text);

  // D) Último recurso: fila Totales por texto
  if (!tot) tot = extractTotalesRow(text);

  // E) Si aun así no, zeros
  if (!tot) {
    tot = {
      original: 0, vigente: 0, saldo: 0, vencido: 0,
      d1_29: 0, d30_59: 0, d60_89: 0, d90_119: 0, d120_179: 0, d180_plus: 0
    };
  }

  // Normaliza estructura (según venga de diferentes extractores)
  const {
    original=0, vigente=0, saldo:saldoIn=null, vencido:vIn=null,
    d1_29: b1=null, d30_59: b30=null, d60_89: b60=null,
    d90_119: b90=null, d120_179: b120=null, d180_plus: b180=null,
    buckets: maybeBuckets=null,
    _debug_coords, _debug_rows, _debug_totalesBest
  } = tot;

  const d1_29     = (maybeBuckets?.v1_29 ?? maybeBuckets?.d1_29 ?? b1)   ?? 0;
  const d30_59    = (maybeBuckets?.v30_59 ?? maybeBuckets?.d30_59 ?? b30) ?? 0;
  const d60_89    = (maybeBuckets?.v60_89 ?? maybeBuckets?.d60_89 ?? b60) ?? 0;
  const d90_119   = (maybeBuckets?.v90_119 ?? maybeBuckets?.d90_119 ?? b90) ?? 0;
  const d120_179  = (maybeBuckets?.v120_179 ?? maybeBuckets?.d120_179 ?? b120) ?? 0;
  const d180_plus = (maybeBuckets?.v180p ?? maybeBuckets?.d180_plus ?? b180) ?? 0;

  const vencido = vIn != null ? Math.round(vIn)
    : Math.round(d1_29 + d30_59 + d60_89 + d90_119 + d120_179 + d180_plus);
  const saldo = saldoIn != null ? Math.round(saldoIn)
    : Math.round((vigente ?? 0) + vencido);
