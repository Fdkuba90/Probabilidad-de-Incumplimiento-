...
/* ========= Califica/Calificador: tabla de IDs (texto) ========= */
const KNOWN_CODES = new Set([
  ...
  // + algunos auxiliares que aparecen en las tablas completas
  "BK12_CLEAN","BK12_NUM_TC_ACT","NBK12_NUM_CRED","NBK12_PCT_SAT",
  "NBK24_PCT_60PLUS","BK_IND_PMOR","BK24_IND_EXP","12_INST",
  "BK_DEUDA_TOT","BK_DEUDA_CP","NBK_DEUDA_TOT","NBK_DEUDA_CP","DEUDA_TOT","DEUDA_TOT_CP"
]);
const ID_WHITELIST = new Set([1,4,5,6,7,9,11,12,13,14,15,16,17]);

/**
 * Busca un bloque que inicie con "Califica" o "Calificador" y luego los encabezados
 * "Identificador de la característica / Código / Valor". Extrae líneas "ID CODE VALUE".
 */
function extractCalificaRows(text) {
  const labelRe = /(Califica(?:dor)?)/i;
  const lines = text.split("\n");

  // Buscamos el primer "Califica" o "Calificador"
  let startIdx = lines.findIndex(l => labelRe.test(l));
  if (startIdx === -1) return [];

  // Ventana razonable hacia adelante
  const windowLines = lines.slice(startIdx, startIdx + 400); // ~bastante
  const scope = windowLines.join("\n");

  // Línea por línea con patrón flexible (soporta espacios extra y "--")
  ...
  while ((m = lineRe.exec(scope))) {
    ...
    let valorRaw = String(m[3] || "").trim();

    // Cortar en caso de que venga más basura a la derecha (otra columna)
    // Tomamos hasta el primer “ doble espacio + mayúsculas + _ ” típico del siguiente código
    const cut = valorRaw.match(/\s{2,}[A-Z0-9_]{2,}\b/);
    if (cut) valorRaw = valorRaw.slice(0, cut.index).trim();

    // Normalizar "--" y similares
    if (/^(--|-|N\/?A|N\.A\.|Sin\s+Información)$/i.test(valorRaw)) valorRaw = "Sin Información";

    // Filtro por códigos/IDs que nos interesan
...
export async function parsePdf(buffer) {
  ...
  const calificaRows = extractCalificaRows(text); // <- ahora robusto para Califica/Calificador

  // Totales de Créditos Activos (por texto con respaldos)
  let tot = sumCreditosActivosByRows(text);
  ...
  const {
    ...
    vencido: vCalc,
    d1_29=0, d30_59=0, d60_89=0, d90_119=0, d120_179=0, d180_plus=0,
    _debug_rows, _debug_totalesBest
  } = tot;
