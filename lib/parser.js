...
function extractCalificaRows(text) {
  // Soporta “Califica” o “Calificador”
  const match = text.match(/Calific(?:a|ador)\b/i);
  let scope = text;
  if (match) {
    const idx = text.indexOf(match[0]);
    scope = text.slice(idx, idx + 20000); // ventana amplia
  }
...
export async function parsePdf(buffer) {
  ...
  const calificaRows = extractCalificaRows(text); // <- regex (sirve para “Califica”)

  // A) Coordenadas primero para Totales (Créditos Activos)
  let tot = await extractCreditosActivosByCoords(buffer).catch(() => null);
  ...
  return { razonSocial, rfc, calificaRows, totales, milesDePesos, text, _debug: debug };
}
