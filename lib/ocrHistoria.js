// lib/ocrHistoria.js — OCR del bloque "Historia:" (Node/Vercel friendly)
import { createWorker } from "tesseract.js";
import * as pdfjs from "pdfjs-dist";           // usar export raíz en v4
import { createCanvas } from "@napi-rs/canvas"; // reemplazo de canvas nativa

// En Node no usamos web worker de pdfjs
...
function getHistoriaOcrBlock(fullText) {
  ...
  if (ms === -1) return "";
  const matchLen = (fullText.slice(ms).match(reStart)?.[0]?.length || 0);
  const rest = fullText.slice(ms + matchLen);
...
export async function ocrHistoriaFromPdf(buffer) {
  ...
  // Revisamos primeras páginas donde suele estar “Historia”
  const pagesToScan = [];
