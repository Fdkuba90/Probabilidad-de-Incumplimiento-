// OCR de "Historia por mes": pdfjs-dist v4 + @napi-rs/canvas + tesseract.js
...
class NapiCanvasFactory {
  ...
  reset(c, width, height) { c.canvas.width = Math.ceil(width); c.canvas.height = Math.ceil(height); }
  destroy(c) { if (!c) return; c.canvas.width = 0; c.canvas.height = 0; }
}
...
function toMilesNumber(tok) {
  const s = String(tok).replace(/[^\d.,\s-]/g,"").replace(/\s+/g,"").replace(/(?<=\d)[.,](?=\d{3}(?:\D|$))/g,"").replace(/,/g,".");
  const n = Number(s); return Number.isFinite(n)&&n>=0 ? Math.round(n*1000) : 0;
}

async function findHistoriaPages(pdf) {
  const pages=[]; for (let p=1;p<=pdf.numPages;p++) {
    const page = await pdf.getPage(p);
    const text = await page.getTextContent();
    const joined = text.items.map(it=>it.str).join(" ").replace(/\s+/g," ");
    if (/Historia\s+por\s+mes\s*\(pesos\)/i.test(joined) || /\bHistoria\b/i.test(joined)) pages.push(p);
  }
  return pages.length? pages : Array.from({length: pdf.numPages},(_,i)=>i+1);
}
...
function parseHistoriaFromOcrText(text) {
  ...
  for (let i=0;i<tokens.length;i++) {
    const start=tokens[i].idx, end = (i+1<tokens.length)? tokens[i+1].idx : text.length;
    const month = tokens[i].token.replace(/\s+/g," "); const body = text.slice(start,end);

    const pickNum = (re) => {
      const m1 = body.match(new RegExp(re.source + "[^\\n\\r]*?([\\d.,\\s]{1,20})", "i"));
  ...
  rows.sort((a,b)=>{ const [ma,ya]=a.month.split(/\s+/),[mb,yb]=b.month.split(/\s+/); const ia=MESES.indexOf(ma), ib=MESES.indexOf(mb); return (ya-yb)||(ia-ib); });
  return { rows: rows.slice(-12), historiaRaw: text };
}

export async function extractHistoriaOCR(pdfBuffer) {
  const loading = pdfjs.getDocument({ data: pdfBuffer });
  const pdf = await loading.promise;

  const pages = await findHistoriaPages(pdf);
  const allRows=[]; let dump=""; const maxPages = Math.min(pages.length, 2);
  ...
  const byMonth = new Map(); for (const r of allRows) byMonth.set(r.month, r);
  const rows = Array.from(byMonth.values()).sort((a,b)=>{ const [ma,ya]=a.month.split(/\s+/),[mb,yb]=b.month.split(/\s+/); const ia=MESES.indexOf(ma), ib=MESES.indexOf(mb); return (ya-yb)||(ia-ib); }).slice(-12);
