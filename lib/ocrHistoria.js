// lib/ocrHistoria.js
// Extrae la sección "Historia por mes (pesos)" por OCR.
// Renderiza páginas del PDF a PNG con @napi-rs/canvas y pdfjs-dist,
// ejecuta OCR (tesseract.js) y parsea meses + importes + calificación.

import { createCanvas } from "@napi-rs/canvas";
import * as pdfjs from "pdfjs-dist/legacy/build/pdf.js";
import Tesseract from "tesseract.js";

// En Node no necesitamos worker separado
...
// Busca páginas candidatas con "Historia" usando el texto nativo de pdf.js
async function findHistoriaPages(pdfDocument) {
  ...
  // Si no detectamos, intentamos todas (pero esto rara vez pasa)
  return pages.length ? pages : Array.from({ length: pdfDocument.numPages }, (_, i) => i + 1);
}

// Renderiza una página a PNG (buffer)
...
async function ocrText(pngBuffer) {
  const { data: { text } } = await Tesseract.recognize(pngBuffer, "spa", {
    // whitelists ayuda a tesseract a no inventar símbolos
...
function parseHistoriaFromOcrText(text) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const pickNum = (labelRe) => {
      // 1) buscar en misma línea
      const lineMatch = body.match(new RegExp(labelRe.source + "[^\\n\\r]*?([\\d.,\\s]{1,20})", "i"));
      if (lineMatch && lineMatch[1]) {
        const v = toMilesNumber(lineMatch[1]);
        if (v >= 0) return v;
      }
      // 2) buscar primeras cifras en las dos líneas siguientes
