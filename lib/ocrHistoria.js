// lib/ocrHistoria.js — OCR del bloque "Historia:" como último fallback
import { createWorker } from "@tesseract.js/worker";
import * as pdfjs from "pdfjs-dist";
import { createCanvas } from "canvas";

// En Node, pdfjs usa worker interno; no necesitas setear workerSrc manual mientras no estés en browser.
// Aun así, para entornos que lo requieren:
...
function parseHistoriaOcrText(txt) {
  ...
  for (let i=0;i<lines.length;i++) {
    ...
    // trata de leer “número + Vigente” o “Vigente + número”
    let vigente = 0;
...
export async function ocrHistoriaFromPdf(buffer) {
  ...
  // OCR de las primeras 6 páginas (suele estar en la 4–5)
  const pagesToScan = [];
  ...
  // sanity check: si todo es 0, no sirve
