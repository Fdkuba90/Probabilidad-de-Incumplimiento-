...
// 1) Coordenadas por encabezados con centros de columna
...
function findActivosPage(data) {
  ...
  for (let i=0;i<pages.length;i++) {
    const rows = pageToRows(pages[i], 0.35);
...
function mapRow(row, centers, maxDist) {
  const acc = {
    ...
    numericByX: [],
    isTotales: row.cells.some(c => /Totales\s*:?\s*$/.test(c.s))
  };
  ...
  // Respaldo: orden fijo izquierda→derecha
  const ordered = acc.numericByX.sort((a,b)=>a.x-b.x).map(o=>o.n); // [O,V,1-29,30-59,...,180+]
  if (ordered.length >= 2) {
  ...
  return { original, vigente, buckets, isTotales: acc.isTotales };
}
...
export async function extractCreditosActivosByCoords(buffer) {
  ...
  const rows = pageToRows((data.Pages || [])[hit.pageIndex], 0.35);
  ...
  for (const r of candidates) {
    ...
    const m = mapRow(r, centers, maxDist);
    if (m.isTotales) {
      ...
      return {
        original: Math.round(safe(m.original)),
        vigente:  Math.round(safe(m.vigente)),
        buckets: {
