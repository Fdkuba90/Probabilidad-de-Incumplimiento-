...
// 4) NUEVO: detectar "Totales" en la fila anterior y números en la fila actual (fusión hacia atrás)
...
// === helpers para “Totales” y números por fila ===
...
export async function extractCreditosActivosByCoords(buffer) {
  ...
  let bestResult = null;
  const mergeTol = 1.6; // un poco más permisivo
  ...
  for (let idx = 0; idx < candidates.length; idx++) {
    ...
    // Caso A (ya lo tenías): “Totales” en la fila actual pero sin números → fusionar con la siguiente
    ...
    // Caso B (NUEVO): “Totales” en la fila anterior y números en la fila actual → fusionar hacia atrás
    if (!m.isTotales && hasTotales(prev) && Math.abs(r.y - prev.y) <= mergeTol) {
      const merged = { y: prev.y, cells: [...prev.cells, ...r.cells] };
      const m2 = mapRow(merged, centers, maxDist);
      if (m2.numCount >= m.numCount) {
        m = { ...m2, isTotales: true }; // forzamos Totales tras fusión
      }
    }
    ...
    if (m.isTotales) {
      ...
      // seguimos por si existiese otro "Totales" más abajo — nos quedamos con el último hallado
    }
  }
